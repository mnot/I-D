...
A HTTP Cache Invalidation Resource (hereafter, 'invalidation resource') is a HTTP resource ({{Section 3.1 of HTTP}}) that has the behaviours described in this section.

Requests to invalidation resources SHOULD require some form of authentication. The specific means of authentication are out of scope for this document, but note that {{desc-format}} allows authentication details to be carried in the description format.

When an invalidation resource receives a POST request whose content is in the format described in {{event-format}}, its expected behaviour will be to process the request and invalidate the indicated responses stored in the cache(s) associated with it.
...
This specification does not define a format for responses from invalidation resources; implementations MAY send responses with empty content. Future extensions might allow description of the results of invalidation.
...
Additionally, this document defines one optional member:
...
Unrecognised members of the object MUST be ignored, to allow future updates of this specification to add new features.
...
To allow easy configuration of origin servers and tools that they use, this document defines a Gateway Description Format that describes the configuration of a HTTP gateway (commonly, a "reverse proxy" or content delivery network). It is intended to be generated by gateways and made available out-of-band to origin servers using them.
